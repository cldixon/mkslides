...
def init():
    click.echo(click.style("Initializing _mkslides_ artifacts", fg="green", underline=True))
    if os.path.exists(DefaultPaths.CONFIG_FILENAME):
        raise FileExistsError(click.style(f"Mkslides configuration file '{DefaultPaths.CONFIG_FILENAME}' already exists. Either delete existing file first or add a new deck section to existing file.", fg='red'))
    else:
        initialize_configuration_file()
        click.echo(click.style(f"- Added new configuration file '{DefaultPaths.CONFIG_FILENAME}'...", fg='green'))
    ...
    if not os.path.exists(DefaultPaths.SLIDE_DIR):
        ...
        click.echo(click.style(f"- Found existing '{DefaultPaths.SLIDE_DIR}' directory...", fg="yellow"))
...
def build():
    ...
    if not os.path.exists(config.output_dir):
        ...
        click.echo(click.style(f"Created directory '{config.output_dir}' to store output slide decks...", fg='yellow'))
